 ...
 */
function getPolynom() {
    let coeff = Array.from(arguments);
    let kol = arguments.length -1;
    return x => coeff.reduce((acc, currVal, currInd)=>{ acc = acc + Math.pow(x, kol - currInd)*currVal; return acc},0)
}
...
function partialUsingArguments(fn) {
    ...
    return function() {
    	return arg.concat(Array.from(arguments)).join('')
    }
}
